...
#phonemes used in English, sil is silent
...
#Scaline audio to values between -1 and 1
...
def get_emg_features(emg_data, debug=False):
    ...
    for i in range(emg_data.shape[1]):
        ...
        #extracting different features using librosa
        w_h = librosa.util.frame(w, frame_length=16, hop_length=6).mean(axis=0) #splits signal into overlapping frames, computes mean value of each frame
        p_w = librosa.feature.rms(y=w, frame_length=16, hop_length=6, center=False) 
        p_w = np.squeeze(p_w, 0)#RMS energy (low frequency)
        p_r = librosa.feature.rms(y=r, frame_length=16, hop_length=6, center=False) 
        p_r = np.squeeze(p_r, 0) #RMS of rectified residual (how rapidly varying the EMG signal is)
        z_p = librosa.feature.zero_crossing_rate(p, frame_length=16, hop_length=6, center=False) 
        z_p = np.squeeze(z_p, 0)#how many times a signal changes sign within each frame, roughness
        r_h = librosa.util.frame(r, frame_length=16, hop_length=6).mean(axis=0) #mean of rectified residual signal r, per frame (average absolute deviation of high frequency)
...
#stores mean and standard deviation of features to normalize data for consistent model input
...
#confusion matrix used to help evaluate commonly confused phonemes
