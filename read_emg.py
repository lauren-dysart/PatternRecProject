#script for preprocessing EMG data
...
flags.DEFINE_list('silent_data_directories', ["C:\Users\r33tm\PatternRecProject\emg_data\closed_vocab\silent"], 'silent data locations') ##change to file directory
flags.DEFINE_list('voiced_data_directories', ["C:\Users\r33tm\PatternRecProject\emg_data\closed_vocab\voiced"], 'voiced data locations')
...
#standard EMG preprocessing steps

#applied a 3rd order highpass Butterworth filter at  2Hz
def remove_drift(signal, fs):
    b, a = scipy.signal.butter(3, 2, 'highpass', fs=fs)
    return scipy.signal.filtfilt(b, a, signal)
#notch filter to remove interferences
def notch(signal, freq, sample_frequency):
    b, a = scipy.signal.iirnotch(freq, 30, sample_frequency)
    return scipy.signal.filtfilt(b, a, signal)
#removes further noise
def notch_harmonics(signal, freq, sample_frequency):
    for harmonic in range(1,8):
        signal = notch(signal, freq*harmonic, sample_frequency)
    return signal
#resamples EMG to lower frequency
def subsample(signal, new_freq, old_freq):
    times = np.arange(len(signal))/old_freq
    sample_times = np.arange(0, times[-1], 1/new_freq)
    result = np.interp(sample_times, times, signal)
    return result
#applies above filters to each EMG channel
...
#loads data for one recorded sentence (utterance)
...
#loading corresponding audio from .flac file
...
#aligns EMG and audio frame lengths
...
#aligned EMG signal with audio, sentence, and phoneme label
...
#shuffles all samples and groups them by length
...
class EMGDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i):
        ...
        raw_emg = raw_emg / 20
        raw_emg = 50*np.tanh(raw_emg/50.) #amplitude compression to limit outliers

        if not self.no_normalizers:
            mfccs = self.mfcc_norm.normalize(mfccs)
            emg = self.emg_norm.normalize(emg)
            emg = 8*np.tanh(emg/8.)
          #ALTER EMG DATA HERE BEFORE RETURNED TO MODEL???
